...
async def monitor_account_task(api_manager, email, msg, chat_id, group_name, is_edit_context=False, account_id=None):
    """
    ...
    ✅ النسخة النهائية المحسّنة:
       ...
       - تدعم is_edit_context للتمييز بين الإضافة والتعديل
       - 🆕 تدعم account_id لتخطي البحث بالإيميل (مهم عند تعديل Email)
    """
    try:
        async with monitoring_semaphore:
            ...
            await wait_for_status_change(
                ...
                chat_id,
                group_name,
                is_edit_context,  # 🆕 تمرير الـ context
                account_id,  # 🆕 تمرير account_id لتخطي البحث
            )
...
async def wait_for_status_change(
    ...
    is_edit_context: bool = False,  # 🆕 NEW PARAMETER
    account_id: Optional[str] = None,  # 🆕 تمرير ID مباشرة لتخطي البحث
    ...
    🆕 عند تعديل حساب (خصوصاً تعديل Email):
    - لو account_id متوفر، نتخطى البحث بالإيميل كلياً
    - نبدأ المراقبة مباشرة بالـ ID (أسرع + يحل مشكلة تعديل Email)
    """
    ...
    status_changes = []
    stable_count = 0
    # account_id = None # This line is removed as account_id is now a parameter

    # 🚀 الخطوة 1: جلب الحساب (أو استخدام ID المُمرر مباشرة)
    
    # 🆕 التحسين: لو account_id متوفر، نتخطى البحث بالإيميل
    if account_id:
        print(f"\n[BURST] ✅ Using provided account_id: {account_id}")
        print(f"[BURST] ⚡ Skipping email search (faster + fixes email edit issue)")
        
        # جلب البيانات بالـ ID مباشرة
        try:
            result = await api_manager.search_sender_by_id(account_id)
            if not result:
                print(f"[BURST] ❌ Account not found by ID: {account_id}")
                return False, None
            print(f"[BURST] 📊 Initial status: {result.get('Status', 'N/A')}")
        except Exception as e:
            print(f"[BURST] ❌ Error getting account by ID: {e}")
            return False, None
    else:
        # الطريقة القديمة: البحث بالإيميل
        print(f"\n[BURST] 🔍 Searching for account: {email}")
        
        result = await api_manager.search_sender_by_email(email)
        
        if not result:
            print(f"[BURST] ❌ Account not found: {email}")
            await message_obj.edit_text(
                f"❌ *خطأ*\n\n"
                f"📧 `{email}`\n\n"
                f"⚠️ الحساب غير موجود في النظام.\n"
                f"تأكد من الإيميل وحاول مرة أخرى.",
                parse_mode="Markdown",
            )
            return False, None
        ...
        if not account_id:
            print(f"[BURST] ❌ No ID found for: {email}")
            return False, None
        
        print(f"[BURST] ✅ Account found - ID: {account_id}")
        print(f"[BURST] 📊 Initial status: {result.get('Status', 'N/A')}")
        ...
        try:
            ...
            if is_final:
                ...
                await send_status_notification(
                    ...
                    chat_id,
                    account_info,
                    "edit" if is_edit_context else "bot",  # 🔧 تمييز المصدر
                )
...
async def send_status_notification(
    ...
    try:
        ...
        # 🔧 تمييز المصدر (إضافة، تعديل، يدوي)
        if source == "edit":
            source_line = "🔧 المصدر: من تعديل"
        elif source == "bot":
            source_line = "🤖 المصدر: من البوت"
        else:
            source_line = "👤 المصدر: يدوي"
